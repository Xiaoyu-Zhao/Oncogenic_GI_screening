...
colnames(singleEffect) = c("PTEN-/-", "PIK3CA", "MYC")
...
#----------------------------------------------------------------------------------------------------
res0 = read.csv("Effects_top8_single_gene_in_tumors_20200908.csv", row.names = 1, header = T)
colnames(res0) = c("MYC", "PIK3CA", "PTEN-/-")
...
my_sample_col <- data.frame(Tumors = c("MYC", "PIK3CA", "PTEN-/-"))
...
my_gene_col <- data.frame(Categories = c(rep("Background_indepedent",3), "Favored_in_PTEN-/-",
                                         "Favored_in_MYC", rep("No_effects_in_PTEN-/-",3)))
...
colnames(unNION) = c("PTEN-/-", "PIK3CA", "MYC")
rownames(unNION) = cen
...
unNION[match(rownames(cen_myc), rownames(unNION)), "MYC"] = cen_myc[,"degree"]
unNION
...
Conditions = factor(data$condition, levels = c("MYC", "PIK3CA", "PTEN-/-"))
...
ggplot(data, aes(x = Conditions, y = snames, size= cen**2 ))+ 
  ...
  theme(panel.border = element_rect(fill=NA, size=1, color = "black"), 
        ...
        axis.text.y  = element_text(size=18.5,  color = 'black' ),
        axis.text.x.top =element_text(size=16, vjust = 0.5, color = 'black', angle = 90),
...
unNION.2 = data.frame(matrix(0, length(nGI), 3)) 
colnames(unNION.2) = c("PTEN-/-", "PIK3CA", "MYC")
rownames(unNION.2) = nGI

unNION.2[match(rownames(gi_pten), rownames(unNION.2)), "PTEN-/-"] = gi_pten[,"Max"]
unNION.2[match(rownames(gi_pik3), rownames(unNION.2)), "PIK3CA"] = gi_pik3[,"Max"]
unNION.2[match(rownames(gi_myc), rownames(unNION.2)), "MYC"] = gi_myc[,"Max"]
unNION.2
#sum of positive interactions
unNION.2$sum_3 = rowSums(unNION.2[,1:3] > 0)
unNION.2$sum_2 = rowSums(unNION.2[,1:2] > 0)
unNION.2$sum_1 = rowSums(unNION.2[,2:3] > 0)
#sum of interaction scores
unNION.2$score_3 = rowSums(unNION.2[,1:3])
unNION.2 = unNION.2[with(unNION.2, order(-sum_3, -sum_2, -sum_1, -score_3)),]
df = unNION.2[,1:3]

df$sname = rownames(df)
#df$sum = rowSums(df[,1:3])
data = gather(df, condition, gi, -sname)
data$gi[data$gi == 0] = NA

snames = factor(data$sname, levels= rownames(unNION.2[with(unNION.2, order(-sum_3, -sum_2, -sum_1, -score_3)),]))
Conditions = factor(data$condition, levels = c("MYC", "PIK3CA", "PTEN-/-"))

ggplot(data, aes(x = Conditions, y = snames, size= gi))+ 
  geom_point(color = "#007AFF")+ scale_size_area(max_size = 8)+
  #scale_fill_gradient(color = "blue")+
  theme(panel.border = element_rect(fill=NA, size=1, color = "black"), 
        panel.grid = element_blank(),
        panel.background = element_blank(),
        ...
        legend.text = element_text(size=20 ),
        axis.text.y  = element_text(size=18.5,  color = 'black' ),
        axis.text.x.top =element_text(size=15, vjust = 0.5, color = 'black', angle = 90),
        axis.title.x=element_text(size=16, color = 'black', face="bold"),
        plot.title = element_text(size=26,hjust= 1, vjust = 3)
        #strip.text.y  = element_text(size = 26, face="bold")
        ##strip.text is for facet
...
ggsave("GI_comparison_3_conditions.pdf", width = 9.6, height = 5.2)
