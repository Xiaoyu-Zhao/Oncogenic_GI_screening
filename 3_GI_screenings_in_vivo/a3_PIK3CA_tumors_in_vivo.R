...
nrow(ggi[which(ggi$oncogenic == "Yes_oncogenic"),]) #55
...
ggi$hL = 0
ggi[which((ggi$LFC > 1.7 & ggi$p_adj < 0.05) & (ggi$Max > 1.5) & (ggi$p.max.adj < 0.05)),"hL"] = 1 ##20
...
ggsave("Diff_SKO_from_DKO_PIK3CA_tumors_wo_text.pdf", width=6.2, height =6.5)
...
df$sample = factor(df$samples, levels = c("Cell.1", "Cell.2", "F995.1", "F995.2","F996.1"))
...
#scale_fil8
ggsave("SKO_DKO_NTC_individual_tumors_PIK3CA_2.png", width = 4.3, height = 5.8, dpi = 300)
