...
filter_flag_1 = sapply(bad_candidate, function(x) {grepl(x, rownames(ct.pten))})
filter_flag_2 = apply(filter_flag_1, 1, sum)
filter_flag = ifelse(filter_flag_2==0, TRUE, FALSE)
counts = ct.pten[filter_flag, 3:ncol(ct.pten)]
dim(counts)
write.csv(counts, "cell_tumor_counts_PTEN.csv")
#In MCF10A PTEN-/- cells, sgPTEN was treated as CTRL
rnm = rownames(counts)
rownames(counts) = gsub("PTEN", "CTRL0", rnm) 
write.csv(counts, "cell_tumor_counts_PTEN_wo_sgPTEN.csv")
#------------------------------------------------------------------------------------------------------------
counts = read.csv('cell_tumor_counts_PTEN_wo_sgPTEN.csv', row.names = 1, header = T)
keep <- rowSums(counts[,1:2]) >= 2  #Keep dual-sgRNAs with initial non-zero counts in cell libraries
cts = counts[keep,]
RPM = as.matrix(apply(cts[,1:ncol(cts)]+0.1, 2, function(x) {x/sum(x)*1000000})) 

#reference gene_comb names for PTEN-/- tumors
sname0 <- ref$gene_comb
sname = sname0[!grepl("PTEN", sname0)]
write.csv(sname, "gene_comb_names_wo_sgPTEN.csv")
...
#Determine significance of effect size (lfc) and confidence interval (CI) using Bootstrap 
set.seed(42)
LFC.Boot = NULL
Boot = NULL
nBoot = 1000
...
write.csv(res.Boot, 'res_LFC_PTEN.csv')
...
counts = read.csv('cell_tumor_counts_PTEN_wo_sgPTEN.csv', row.names = 1, header = T)
ref <- read.csv("gene_comb_names_wo_sgPTEN.csv",row.names = 1)
...
write.csv(log2rpm, "log2rpm_PTEN.csv")
#-------------------------------------------------------------------------------------------------------------
#Calculate the mean log2RPM at gene level
c.gg <- c() ##average of log-relative-freq of each gene pair in cells
t.gg <- c()##average of log-relative-freq of each gene pair in tumors
...
write.csv(m0, "Outlier_sgRNA_level_PTEN.csv")
...
#ggsave("logRPMtumor.vs.cell_20200908_wo_PTEN_Bootstrap_permutation.pdf", width = 7, height =8)

#---------------------------------IV. Examine how many dual-sgRNAs enriched--------------------------------------
rP = read.csv("Outlier_sgRNA_level_PTEN.csv", row.names = 1)
rAvg = read.csv("Outlier_gene_level_PTEN.csv", row.names = 1)
...
write.csv(rAvg, "Number_outlier_sgRNAs_PTEN.csv")
...
read = read.csv('res_LFC_PTEN.csv', row.names = 1, header = T)
lfc1 = t(apply(read, 1, function(x){(x[3:11])/(x[1])}))
lfc2 = t(apply(read, 1, function(x){(x[3:11])/(x[2])}))
...
GGIcompute = function(doubleK, singleM = readS) {
  ...
  lfc.gg = as.numeric(doubleK[20])
  g1 = strsplit(sname, split = '-')[[1]][1]
  g2 = strsplit(sname, split = '-')[[1]][2]
  lfc.g1 = as.numeric(singleM[grepl(g1,rownames(singleM)),19][!is.na(singleM[grepl(g1,rownames(singleM)),19])])
  lfc.g2 = as.numeric(singleM[grepl(g2,rownames(singleM)),19][!is.na(singleM[grepl(g2,rownames(singleM)),19])])
...
##Construct the DKO and SKO matrices (ncol = 18 + 18 = 36, nrow = 1325 gene pairs)
#Maxmium model
LFC.max = function(doubleK, singleM = readS){
  ...
  lfc.gg = as.numeric(doubleK[2:19])
  lfc.gmax = as.numeric(singleM[grepl(gmax.sname,rownames(singleM)),1:18])
  lfcM = matrix(c(lfc.gg, lfc.gmax), nrow = 1)
  colnames(lfcM) = c(paste0('DKO_',names(doubleK[2:19])), paste0('Max_SKO_',names(doubleK[2:19])))
  rownames(lfcM) = sname
  return(data.frame(lfcM))
}

#Optional additive model
LFC.Add = function(doubleK, singleM = readS){
  ...
  lfc.gg = as.numeric(doubleK[2:19])
  lfc.add = as.numeric(singleM[grepl(g1,rownames(singleM)),1:18]) + as.numeric(singleM[grepl(g2,rownames(singleM)),1:18])
  lfcM = matrix(c(lfc.gg, lfc.add), nrow = 1)
  colnames(lfcM) = c(paste0('DKO_',names(doubleK[2:19])), paste0('Add_SKO_',names(doubleK[2:19])))
  rownames(lfcM) = sname
  return(data.frame(lfcM))
}
...
#Determine the significance of GIs by comparing 18 ratios from 9 tumor samples and 2 cell samples using Bootstrap_permutation methods
set.seed(42)
max.Boot = NULL
add.Boot = NULL
nBoot = 1000
...
write.csv(GGI, 'GI_score_PTEN.csv')

#----------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@-------------------------------------------------
#==========================Part 5: Combine outlier results with LFC and GI score=================================
#----------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@-------------------------------------------------
Outlier = read.csv("Number_outlier_sgRNAs_PTEN.csv", row.names = 1 )
dim(Outlier)
cN = paste0("OT_",colnames(Outlier))
colnames(Outlier) = cN
#--------------------------I. Combine LFC file and number of outliers file-------------------------------------
res = read.csv("res_LFC_PTEN.csv", row.names =1)
sum(!(rownames(res) %in% rownames(Outlier)))
gP1 = cbind.data.frame(res,Outlier[rownames(res),])
dim(gP1)
write.csv(gP1,"Res_LFC_Outlier_PTEN_1327_combs.csv" )
#--------------------------II. Combine GI file and number of outliers file-------------------------------------
GGi = read.csv("GI_score_PTEN.csv", row.names =1)
sum(!(rownames(GGi) %in% rownames(Outlier)))
dim(GGi)
gP2 = cbind.data.frame(gP1[rownames(GGi),], GGi)
dim(gP2)
write.csv(gP2,"Final_LFC_Outlier_GI_score_PTEN_1275_DKOs.csv")

#----------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@-------------------------------------------------
#=====================Part 6: Analysis or plotting after determination of LFC and GI score======================
#----------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@-------------------------------------------------
#----------------------------------------I. LFC_based_on_gene_rank----------------------------------------------
lfc_pten = as.data.frame(read.csv("Res_LFC_Outlier_PTEN_1327_combs.csv", header = T, row.names = 1))
dim(lfc_pten)
lfc_pten$sname = rownames(lfc_pten)
lfc_pten$hL = "not.hL"
lfc_pten[which(lfc_pten$LFC > 0 & lfc_pten$p_adj < 0.05 ),"hL" ] = "hL"
lfc_pten$Order[order(lfc_pten$LFC, decreasing = T)] = 1:nrow(lfc_pten)
data = lfc_pten[,c("Order", "LFC", "p_adj","OT_perP", "hL")] 
sum(data$hL == "hL") #71
...
p <- ggplot(data, aes(x = Order, y = LFC))+
  ...
  ##strip.text is for facet)+
  labs(title = "PTEN-/-",x = "Rank of gene pairs", y = expression(paste(log[2]~fold~change, " (LFC)")))+
...
p  
ggsave("LFC_based_on_gene_rank_PTEN.pdf", width = 5.2, height = 5.8)

#--------------------------- II. Scatter plot of Avg.log2RPM in tumors vs in cells--------------------------------------------
f1 = lfc_pten[which(lfc_pten$LFC > 0 & lfc_pten$p_adj < 0.05), ]
hl.names = as.character(f1[order(f1[,"LFC"], decreasing = T), "sname"][1:6])
lfc_pten$text.or.not = "not.texted"
lfc_pten[hl.names, "text.or.not"] = "texted"
#Scatterplot of log2RPM in tumors versus log2RPM in cells
p <- ggplot(lfc_pten, aes(x=OT_c.gg, y=OT_t.gg)) +
  ...
  labs(title = "PTEN-/-",x = expression(paste(Avg.~log[2]~RPM, " in cells")), y = expression(paste(Avg.~log[2]~RPM, " in tumors")))+
  scale_x_continuous(limits = c(1,8),breaks = round(seq(1,8, by = 1),0)) +
  scale_y_continuous(limits = c(-1.5,8),breaks = round(seq(-1, 8, by = 1),0))+
  geom_hline(yintercept=1, linetype="dashed", color = "grey")
p
require("ggrepel")
set.seed(68)
p+ geom_text_repel(
  data = subset(lfc_pten, text.or.not == "texted"),
  aes(label = sname),
  size = 6,
  color = "darkblue",
...
ggsave("AvglogRPMtumor.vs.cell_PTEN.pdf", width = 5.2, height = 5.8)

#-------------------------------III. Top 10 tumor-promoting gene pairs----------------------------------------------
res = lfc_pten[which(lfc_pten$hL == "hL"),]
res = res[with(res, order(LFC, decreasing = T)),]
res0 = head(res, 10)

nM.SKO = unique(unlist(strsplit(rownames(res0), split = '-')))
Res = rbind.data.frame(res0, lfc_pten[paste0(nM.SKO, '-CTRL'),])
...
p <- ggplot(dat, aes(x = sgs, y=LFC,fill = group)) + #fill = group
  ...
  labs(title="Top 10 tumor-promoting DKOs",x ="", y = "LFC (PTEN-/-)")+
  coord_flip()+
  geom_hline(yintercept=0, color = "grey5")

p
ggsave('Top10_tumor_promoting_DKOs_PTEN.pdf', width = 5.6, height = 6.2)

#-------------------------------IV. Correlation of LFC and outlier_residual----------------------------------------------
library("ggpubr")
ggscatter(lfc_pten, x = "LFC" , y = "OT_rstudent", size = 3,alpha= 0.5,
          add = "reg.line", conf.int = FALSE, 
          add.params = list(color = "#007AFF", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE,rug = TRUE,
          cor.coeff.args = list(method = "pearson", label.x = -5,label.y = 11, label.sep = "\n", size = 7))+
        theme(#panel.border = element_blank(), 
        panel.grid = element_blank(),
        panel.background = element_blank(),
        #panel.grid= element_line(linetype = "dashed"),
  ...
  #scale_y_continuous(limits = c(2,12.5), breaks = seq(2,12,2))+
  scale_x_continuous(limits = c(-5.5,8.5), breaks = seq(-7.5,10,2.5))+
  scale_y_continuous(limits = c(-1.5,13), breaks = seq(-2.5,12.5,2.5))+
  labs(title="",x ="LFC (PTEN-/-)", y = "Studentized residual")

ggsave("Correlation_outlier_analysis_LFC_PTEN.pdf", width = 6, height =5)

#-------------------------------V. Tumor-promoting GIs will be selected and plotted----------------------------------------------
ggi = read.csv("Final_LFC_Outlier_GI_score_PTEN_1275_DKOs.csv", row.names = 1, header = T)
ggi = as.data.frame(ggi)
ggi$sname = rownames(ggi)
ggi$oncogenic = "Not_oncogenic"
##Oncogenic gene pairs
ggi[which(ggi$LFC > 0 & ggi$p_adj < 0.05 ),"oncogenic"] = "Yes_oncogenic"   ##69
...
ggi$hL = 0
ggi[which((ggi$oncogenic == "Yes_oncogenic") & (ggi$Max > 0) & (ggi$p.max.adj < 0.05)),"hL"] = 1  #18
nrow(ggi[which(ggi$hL == 1),])
d0 = ggi[which(ggi$hL == 1),]
write.csv(d0, "18_Tumor_promoting_GIs_PTEN.csv")
...
p = ggplot(ggi, aes(x = diff1, y = diff2))+
  ...
  labs(title="PTEN-/-",x =expression(paste("Diff: ",LFC[(DKO)]," - ", LFC[(SKO1)])), y = expression(paste("Diff: ",LFC[(DKO)]," - ", LFC[(SKO2)])))+
...
ggsave("Diff_SKO_from_DKO_PTEN_tumors_wo_text.pdf", width=6.2, height =6.5)

#--------------------------VI.Tumor-promoting GIs are selected for cytoscape (GI network map)---------------------------------------------
library(igraph)
apple_colors = c("#5AC8FA", "#FFCC00", "#FF9500", "#FF2D55", "#007AFF", "#4CD964", "#FF3B30",
                 "#8E8E93", "#EFEFF4", "#CECED2", "#000000", "007AFF")
read0 = read.csv("18_Tumor_promoting_GIs_PTEN.csv", row.names = 1, header = T)
dim(read0)
...
write.csv(PPI_net, "GI.score_PTEN_cytoscape.csv", quote = FALSE)
...
plot(net_clean, layout = l,  vertex.frame.color=NA, vertex.shape="circle",vertex.label.color = apple_colors[11],
     vertex.label.cex = 0.8, margin = c(0,0,0,0),main = "PTEN", vertex.label.family= "Arial")
# pdf("22_oncogenic_ggi_PTEN.pdf", height =6, width = 6)
# plot(net_clean, layout = l,  vertex.frame.color=NA, vertex.shape="circle",vertex.label.color = apple_colors[11],
#      vertex.label.cex = 0.78, margin = c(0,0,0,0),main = "PTEN", vertex.label.family= "Helvetica")
# 
# dev.off()
...
write.csv(cent_scores, "Centrality_scores_PTEN.csv")

#-------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@----------------------------------------------
#=====================================Additional analysis==============================================
#-------------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@----------------------------------------------
#-------------------VII. Percentage of dual-sgRNAs enriched in 18 oncogenic GIs-----------------------
library(dplyr)
library(scales)
sum = 1+4+3+4+6
dat = data.frame(group = c("g1", "g2", "g3", "g4", "g5"),
                 num = c(1,4,3,4,6),
                 #num = c(7,13,17,8,1),
                 per = round(c(1,4,3,4,6)/18*100, 1)
...
ggplot(dat, aes(x = 2, y = per, fill = group))+
  ...
  labs(title="PTEN-/-",x ="", y = "")
ggsave("Percentage_of_dual-sgRNAs_enriched_of_18_oncogenicGIs_PTEN.pdf", width=10, height = 5)

#-------------------------VIII. Deseq & tumor sample distance for heatmap--------------------------------------
library(DESeq2)
library(pheatmap)
library( gplots )
counts = read.csv("cell_tumor_counts_PTEN.csv", header=T, row.names=1)
...
pdf("Heatmap_sample_distances_PTEN.pdf",width = 8, height=8)
colours = colorRampPalette( rev(brewer.pal(9, "Blues")) )(128)
heatmap.2(sampleDistMatrix, trace="none", col=colours, margins=c(10,10),cexRow=2,cexCol=2)
...
library(tidyr)
set.seed(10)
raw_cts = read.csv("cell_tumor_counts_PTEN.csv", row.names = 1) ###without two columns of mean_log2RPM
keep <- rowSums(raw_cts[,1:2]) > 0 
cts = raw_cts[keep,]

RPM = as.matrix(apply(cts[,1:ncol(cts)], 2, function(x) {x/sum(x)*1000000})) 
dim(RPM)
write.csv(RPM, "RPM_PTEN.csv")
...
df = df[df$sg_type1 %in% c("DKO", "SKO"),]
df = df[order(df$sg_type1),]
df$sample = factor(df$samples, levels = c("Cell.1", "Cell.2", "F911.3", "F912.2", "F912.3", "F913.1", "F913.2","F913.4", "F912.1", "F912.4", "F913.3"))
ggplot(df, aes(x = sample, y = log2RPM, color = sg_type1))+
  ...
  scale_color_manual(values = c("orchid1", "blue", "lightgreen"))+
...
#scale_fil8
ggsave("SKO_DKO_CTRL_individual_tumors_PTEN_wo_NTC.png", width = 5.8, height = 5.8, dpi = 300)
