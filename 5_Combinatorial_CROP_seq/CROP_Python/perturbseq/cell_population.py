...
class CellPopulation:
    ...
    def __init__(self, matrix, cell_list, gene_list, source='arrays', normalized_matrix=None,normalized_count_Deseq = None,  calculate_statistics=True):
        ...
        #self.normalized_count_Deseq = normalized_count_Deseq
        ...
        if calculate_statistics:
            ...
            gene_list["num_cells_exisit"] = matrix.apply(lambda x: np.sum(x > 0)) #To be consistent with min.cells in Seurat
        ...
        if calculate_statistics:
            ...
            self.guides = sorted(cell_list['guide_identity'].unique())
    ...
    def from_file(cls, directory, genome=None, filtered=True, raw_umi_threshold=2000):
        ...
        if os.path.isdir(os.path.join(directory, os.path.normpath("outs/filtered_gene_bc_matrices"), genome)):
            ...
            if filtered:  #(I used filtered bc matrices from CellRanger)
        ...
        gene_list_ori = pd.read_csv(genes_path, sep='\t', header=None, names=['gene_id', 'gene_name','genre'])
        gene_list = gene_list_ori.query('genre == "Gene Expression"')
        ...
        matrix_ori = mmread(os.path.join(matrix_directory, "matrix.mtx"))
        
        print('Densifying matrix...')
        if filtered:
            matrix_ori = pd.DataFrame(matrix_ori.transpose().todense(),
                                  columns=gene_list_ori['gene_id'],
                                  index=cell_barcodes['cell_barcode'],
                                  dtype='int32')
            matrix = matrix_ori[gene_list_ori['gene_id']]
        ...
        guide_identities = pd.read_csv(os.path.join(directory, 'outs', identity_filename)).set_index('cell_barcode')
...
#xyz's note: pd.index returns index object instead of list, df.columns.tolist or df.columns.values
                                 self.cells.loc[new_matrix.index.values],
#xyz's note: pd.columns returns index object instead of list, df.columns.tolist or df.columns.values
                                 self.genes.loc[new_matrix.columns.values],
        ...
        if normalized_matrix == 'inherit':
            ...
            new_pop.normalized_matrix = self.normalized_matrix.loc[new_matrix.index,:].copy()
        else:
        ...
        if not self.genes['in_matrix'].all():
            ...
            new_pop.genes = new_pop.genes.reindex(self.genes.index.values)
    ...
    def groupby(self, key_name, densify=True, **kwargs):
        ...
        #if not data_normalized and isinstance(self.matrix, pd.SparseDataFrame) and densify:(I changed it since pd.SparseDataFrame is deprecated) and https://stackoverflow.com/questions/21506864/how-does-scipy-sparse-issparse-work-it-always-return-false
        ...
        elif data_normalized and issparse(self.normalized_matrix) and densify:
            sparsify = True
    ...
    def average(self, key_name, verbose=False, **kwargs):
        ...
        cell_list = self.groupby_map(key_name,
                                    {'num_cells': lambda x: x.shape[0],
                                     'UMI_mean': lambda x: x.mean().sum()},                                    
                                    verbose=verbose, **kwargs)
        #gene_list = self.genes.loc[matrix.columns.values, 'gene_name'][['gene_name']].copy()
        gene_list = self.genes.loc[matrix.columns.values, ['gene_name']].copy()
        ...
        if not self.genes['in_matrix'].all():
            ...
            mean_pop.genes = mean_pop.genes.reindex(self.genes.index)
