...
d = data[,c(1:8,149:ncol(data))]
dim(d)
#----------------------------------I. Synergy of 8 common synergistic downregulated genes in NF2-TP53 DKO-----------------------------------------------
unique(d$YAP1_CNA)
...
ggplot(df,  aes(x = catergory, y = expr, fill = catergory)) +
  ...
  geom_hline(yintercept=seq(7.5, 9.5, 0.5), linetype="dashed", color = "grey20")
  #scale_y_continuous(limits = c(8.2,9.5), breaks = seq(8.5, 9.5, 0.5))
  #ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_9_common_synergistic_genes_in_NF2_TP53.pdf", width = 4.4, height = 2.5)
  ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_9_common_synergistic_genes_in_NF2_TP53_wo_legend.pdf", width = 1.9, height = 2.5)

#t.test(c(exp1,exp2), exp3, alternative = "greater")
t.test(c(exp1,exp2), exp3, alternative = "less") #0.0146
t.test(c(exp1,exp2), c(exp4, exp5), alternative = "less") #0.001687

#--------------------------------II. Synergy of 8 common synergistic downregulated genes in NF2-PTEN DKO-----------------------------------------------
...
ggplot(df,  aes(x = catergory, y = expr, fill = catergory)) +
  ...
  labs(title="",x ="", y = "Mean_expression")+
  guides(fill=guide_legend(title="Types of Genetic Alterations"))+
  geom_hline(yintercept=seq(8., 9.5, 0.5), linetype="dashed", color = "grey20")
...
#ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_140_common_synergistic_genes_in_NF2_PTEN.pdf", width = 5, height = 2.5)
ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_9_common_synergistic_genes_in_NF2_PTEN_wo_legend.pdf", width = 1.9, height = 2.5)

t.test(c(exp1,exp2), c(exp3,exp4), alternative = "less") #0.0886
t.test(c(exp1,exp2), exp5, alternative = "less") #0.0003028

#----------------------------------III. Synergy of 8 common synergistic downregulated genes in PTEN-TP53 DKO-----------------------------------------------
unique(d$PTEN_WT_MUT)
unique(d$PTEN_CNA)
...
#ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_140_common_synergistic_genes_in_PTEN_TP53.pdf", width = 4.3, height = 2.5)
ggsave("CROP_R/4_Transcriptional_interactions/Synergy_mean_expression_9_common_synergistic_genes_in_PTEN_TP53_wo_legend.pdf", width = 1.9, height = 2.5)

t.test(exp1, exp2, alternative = "less") #0.2969
t.test(exp1, exp3, alternative = "less")  #0.004803
