 ...
 p <- ggplot(gsdata, aes_(x = ~x)) + xlab(NULL) +
  ...
  p.res <- p.res + ylab("RES" )+
    theme(panel.border = element_blank(), 
          ...
          legend.text = element_text(size=30, color = "black"), #30 or (18 only for S4 downrepresentation)
          ...
          plot.title = element_text(size=31,hjust=0.5, vjust = 1.5,color = "black"),
          plot.margin=margin(t = .5, r = .5, b=.5, l=.5, unit="cm")
          #strip.text.y  = element_text(size = 26, face="bold")
          ##strip.text is for facet
    ) +scale_x_continuous( limits = c(0,17000), breaks = seq(0, 15000, by = 5000)) +
