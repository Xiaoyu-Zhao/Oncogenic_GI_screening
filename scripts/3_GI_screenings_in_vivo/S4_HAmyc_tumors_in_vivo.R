...
p <- ggplot(data, aes(x = Order, y = LFC))+
  ...
  ##strip.text is for facet)+
  labs(title = "HAMyc",x = "Rank of gene pairs", y = expression(paste(log[2]~fold~change, " (LFC)")))+
...
p <- ggplot(lfc_myc, aes(x=OT_c.gg, y=OT_t.gg)) +
  ...
  labs(title = "HAMyc",x = expression(paste(Avg.~log[2]~RPM, " in cells")), y = expression(paste(Avg.~log[2]~RPM, " in tumors")))+
...
p <- ggplot(dat, aes(x = sgs, y=LFC,fill = group)) + #fill = group
  ...
  labs(title="Top 10 tumor-promoting DKOs",x ="", y = "LFC (HAMyc)")+
  coord_flip()+
...
ggscatter(lfc_myc, x = "LFC" , y = "OT_rstudent", size = 3, alpha = 0.5,
  ...
  labs(title="",x ="LFC (HAMyc)", y = "Studentized residual")
...
ggi[which(ggi$LFC > 0 & ggi$p_adj < 0.05 ),"oncogenic"] = "Yes_oncogenic"   ##57
...
p = ggplot(ggi, aes(x = Max, y = Min))+
  ...
  labs(title="HAMyc",x =expression(paste("Diff: ",LFC[(DKO)]," - ", LFC[(SKO1)])), y = expression(paste("Diff: ",LFC[(DKO)]," - ", LFC[(SKO2)])))+
  scale_x_continuous(limits =c(-6,10.5), breaks = seq(-5,10,2.5)) +
  scale_y_continuous(limits= c(-4,7.5),breaks = seq(-2.5,10,2.5))+
...
ggsave("Max_Min_GGi_HAMyc_tumors.pdf", width=6.2, height =6.8)
...
sample = factor(df$samples, levels = c("Cell.1", "Cell.2", "F905.2","F908.4", "F908.3", "F908.1","F905.4.1","F905.4.2"))
...
#scale_fil8
ggsave("SKO_DKO_NTC_individual_tumors_HAMyc.pdf", width = 5.5, height = 5.8)
