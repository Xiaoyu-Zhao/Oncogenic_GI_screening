...
#Input: sheet1 in TableS5 ---> full.1; sheet2 in TableS5 ---> full.2
cond = "full.1"
read = data.frame(read_excel("TableS5_Raw_and_processed_data_combinatorial_screening_in_vitro.xlsx", 1))
...
#Save results
write.csv(cbind(rpm, fitlm), file=paste("full/fitness/",cond, ".RPM_fitlm.csv", sep=""))
...
set.seed(250)
cond = "full.1"
fitlm = read.csv(file=paste("full/fitness/",cond, ".RPM_fitlm.csv", sep=""), header =T)
range(fitlm[,9])
dim(fitlm)
#Remove some noise, and show representative lineages
rd1 = fitlm[which(abs(fitlm[,9]) > 0.02),]
dim(rd1)
rd2= rd1[which((rd1[,9]>0 & rd1[,3] > rd1[,2]) | (rd1[,9]<0 & rd1[,3] < rd1[,2])) ,]
dim(rd2)
ns = sample(nrow(rd2),5000)
read0 = rd2[ns,]
trajM =  traj_Transformation(read0, sname_pos = 1, D0_pos = 2, fitlm_pos = 9)
traj_Plot(trajM, paste("full/fitness/",cond, ".lineage_trajectories.pdf", sep=""))
...
perM_cal = function(cond){
  rpm = read.csv(file=paste("full/fitness/",cond, ".RPM_fitlm.csv", sep=""), header =T, row.names = 1)
...
perM.1 = perM_cal(cond = "full.1")
perM.2 = perM_cal(cond = "full.2")
...
write.csv(perM, "full/fitness/full_relative_RPM_percentage_SKO_DKO_CTRL.csv")
...
totalcond = c("full.1", "full.2")
for (cond in totalcond) {
  read0 = read.csv(paste("full/fitness/",cond, ".RPM_fitlm.csv", sep=""), header=T, row.names=1)
  ...
  write.csv(fitM, file=paste("full/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""))
}

#Construct fitness matrix for bootstrapping or permutation
#---------------------------------------------------------------------------------------------------------------------------------
cond = "full.1"
fitM0.1 = read.csv(file=paste("full/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""), row.names = 1, header = T)
...
cond = "full.2"
fitM0.2 = read.csv(file=paste("full/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""), row.names = 1, header = T)
...
dim(fitM)
write.csv(fitM, file="full/fitness/full.combined.fitMatrix_1379_gene_pairs.csv")

#---------------------------------------------------------------------------------------------------------------------------------
nBoot = 1000
cond = "full.1"
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for full.1
Boot1 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for full.1
idxF1 = 1:49
...
write.csv(RES1, file=paste("full/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))

#---------------------------------------------------------------------------------------------------------------------------------
cond = "full.2"
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for full.2
Boot2 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for full.2
idxF2 = 50:98
...
write.csv(RES2, file=paste("full/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))

#---------------------------------------------------------------------------------------------------------------------------------
cond = "full.combined"

#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for full.1 and full.2
Boot3 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for full.1 and full.2
idxF3 = 1:98
idxR3 = 99:ncol(fitM)
...
write.csv(RES3, file=paste("full/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))
...
for (cond in totalcond) {
  ...
  write.csv(wholeM, file=paste("full/fitness/", cond, "_fitMatrix_SKO_sgRNAs.csv", sep=""))
}
...
cond = 'full.1'
read0 = read.csv(file = paste0("full/fitness/",cond,".RPM_fitlm.csv"), header=T, row.names=1)
wholeM = as.matrix(read.csv(file = paste0("full/fitness/",cond,"_fitMatrix_SKO_sgRNAs.csv"), header=T, row.names=1))
...
write.csv(GGIvalues, file=paste("full/GI/", cond, "_GI_sgRNA_level.csv",sep=""))
...
write.csv(total, file=paste("full/GI/", cond, "_GI_gene_level.csv",sep=""))
...
Fit = Order_rname(file_name = "full/fitness/full.combined.fitlm_se_CI_diff_p_1379_gene_pairs.csv")
dim(Fit)                                   
write.csv(Fit, "full/fitness/final_full_combined_fitlm_se_CI_diff_p_1379_gene_pairs_ordered.csv")

#GI score file: Change rownames by alphabetic order
GI_full.1 = Order_rname(file_name = "full/GI/full.1_GI_gene_level.csv")
colnm = colnames(GI_full.1)
colnames(GI_full.1) = paste0("Rep1_", colnm)
dim(GI_full.1)
GI_full.1 = GI_full.1[order(rownames(GI_full.1)),]
write.csv(GI_full.1, "full/GI/final_full.1_GI_gene_level_ordered.csv")

GI_full.2 = Order_rname(file_name = "full/GI/full.2_GI_gene_level.csv")
colnm = colnames(GI_full.2)
colnames(GI_full.2) = paste0("Rep2_", colnm)
dim(GI_full.2)
GI_full.2 = GI_full.2[order(rownames(GI_full.2)),]
write.csv(GI_full.2, "full/GI/final_full.2_GI.gene_level_ordered.csv")

#Combine fitness and GI score
all(rownames(GI_full.1) == rownames(GI_full.2))
Res = cbind.data.frame(Fit[rownames(GI_full.1),],GI_full.1, GI_full.2)
...
write.csv(Res, "full/Final_full_f.avg_GI.csv")
...
fit = read.csv("full/fitness/final_full_combined_fitlm_se_CI_diff_p_1379_gene_pairs_ordered.csv", row.names = 1, header = T)
dat = data.frame(fit[grepl("CTRL", rownames(fit)),])
dat$sname = unlist(lapply(rownames(dat), function(x) strsplit(x, split = "-")[[1]][1]))
write.csv(dat, "full/fitness/full_SKO_fitness_gene_level.csv")
...
p <- ggplot(dat,aes(x=sname,y=f.avg))+
  ...
  scale_y_continuous(limits = c(-0.17,0.07), breaks = round(seq(-0.2,0.1,0.1),2))+
  coord_flip()+
  labs(title="Full",x ="SKOs", y = "Fitness")
p
ggsave("full/fitness/full_SKO_fitness_53_genes.pdf", width = 3.6, height= 9)

#----------------------------------II. Correlation of gene-level fitness of two replicates--------------------------------
f1 = read.csv("full/fitness/full.1.fitlm_se_CI_diff_p_1379_gene_pairs.csv", row.names = 1, header = T)
f2 = read.csv("full/fitness/full.2.fitlm_se_CI_diff_p_1379_gene_pairs.csv", row.names = 1, header = T)
...
ggscatter(df, x = "rep1" , y = "rep2", size = 3, alpha = 0.5,
          ...
          cor.coeff.args = list(method = "pearson", label.x = -0.12,label.y = 0.1, color = "blue",label.sep = "\n", size = 7))+
  theme(#panel.border = element_blank(), 
    panel.grid = element_blank(),
    panel.background = element_blank(),
    #panel.grid= element_line(linetype = "dashed"),
  ...
  scale_x_continuous(limits = c(-0.15,0.17), breaks = round(seq(-0.15,0.15,0.15),2))+
  scale_y_continuous(limits = c(-0.15,0.15), breaks = round(seq(-0.15,0.15,0.1),2))+
  geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
  geom_vline(xintercept= 0, linetype="dashed", color = "grey")+
  labs(title="Full",x ="Fitness, Rep 1", y = "Fitness, Rep 2")

ggsave("full/fitness/Reproducibility_gene_level_fitnesss_full.pdf", width =4.4, height = 4)

#---------------------------------- III. Plotting oncogenic GIs--------------------------------
res=  data.frame(read.csv("full/Final_full_f.avg_GI.csv", header=T, row.names=1))
...
write.csv(PPI_net, "full/GI/Oncogenic_GIs_cytoscape_full.csv", quote = FALSE)
...
p <- ggplot(res, aes(x = gi.Weighted.Avg.Max, y = gi.Weighted.Avg.Min)) +
    ...
    labs(title="Growth-promoting interactions\n(Full)",x ="Mean: f(DKO) - f(SKO1)", y = "Mean: f(DKO) - f(SKO2)")+
    scale_x_continuous(limits =c(-0.17,0.12), breaks = round(seq(-0.15, 0.1, 0.05),2)) +
    scale_y_continuous(limits= c(-0.07,0.11), breaks = round(seq(-0.05, 0.1, 0.05),2))+
    geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
    geom_vline(xintercept= 0, linetype="dashed", color = "grey")
    p
    require("ggrepel")
...
ggsave("full/GI/GI_scatter_plot_full.pdf", width = 6.5, height = 7)
...
ggscatter(res, x = "Rep1_Max.mean" , y = "Rep2_Max.mean", size = 3, alpha =0.5,
          ...
          add.params = list(color = "#007AFF", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE,rug = TRUE,
          cor.coeff.args = list(method = "pearson", label.x = -0.14,label.y = 0.03, color = "blue",label.sep = "\n", size = 7))+
  theme(#panel.border = element_blank(), 
    panel.grid = element_blank(),
    panel.background = element_blank(),
    #panel.grid= element_line(linetype = "dashed"),
  ...
  scale_x_continuous(limits = c(-0.15,0.06), breaks = round(seq(-0.15,0.05,0.1),2))+
  scale_y_continuous(limits = c(-0.15,0.05), breaks = round(seq(-0.15,0.05,0.05),2))+
  geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
  geom_vline(xintercept= 0, linetype="dashed", color = "grey")+
  labs(title="Full",x ="GI score, Rep 1", y = "GI score, Rep 2")

ggsave("full/GI/Reproducibility_gene_level_GI_score_full.pdf", width = 4.4, height = 4)
...
##Input matrix
input_s = read.csv("full/Final_full_f.avg_GI.csv", row.names=1)
...
for (svalue in totalvalues) {
  ...
  write.csv(GImatrix, file=paste("full/GI/", svalue, ".GIMatrix.csv",sep=""))
  write.csv(corrMatrix, file=paste("full/GI/", svalue, ".correlation.Based.On.GI.csv",sep=""))
  
  ##Determine color of sinlge-gene fitness
  f0 = data.frame(read.csv("full/fitness/full_SKO_fitness_gene_level.csv", row.names = 1, header = T))
  ...
  #GI profiling 
  png(filename = paste("full/GI/", svalue, ".correlation.Based.On.GI.png",sep=""),width=1000, height=1000)
...
ggsave("full/GI/full_scale_color_GI_correlation_r.pdf", width = 3, height =3)
...
ggsave("full/GI/full_scale_color_single_gene_fitness.pdf", width = 3, height =3)
