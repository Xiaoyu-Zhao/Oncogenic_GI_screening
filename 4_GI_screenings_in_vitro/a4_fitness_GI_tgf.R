...
#Input: sheet5 in TableS5 ---> tgf.1; sheet6 in TableS5 ---> tgf.2
cond = "tgf.2"  
read = data.frame(read_excel("TableS5_Raw_and_processed_data_combinatorial_screening_in_vitro.xlsx", 6))
rownames(read) = read[,"gene_names"]
dat00 = as.matrix(read[,3:ncol(read)])
# 1. filter some bad sgRNAs 
filtered_list_others = c("CTRL0003","ARID5B_2", "ARID5B_3","PTEN_1", "PTEN_2", "PTEN_3")
...
write.csv(cbind(rpm, fitlm), file=paste("tgf/fitness/",cond, ".RPM_fitlm.csv", sep=""))
...
traj_Plot = function(trajM, output){
  ...
  ggsave(output,  width = 6, height = 4, device=cairo_pdf)     
}

set.seed(25)
cond = "tgf.1"
fitlm = read.csv(file=paste("tgf/fitness/",cond, ".RPM_fitlm.csv", sep=""), header =T)
range(fitlm[,9])
dim(fitlm)
#Remove some noise, and show representative lineages
rd1 = fitlm[which(abs(fitlm[,9]) > 0.02),]
dim(rd1)
rd2 = rd1[which((rd1[,9]>0 & rd1[,3] > rd1[,2] & rd1[,4] > rd1[,3] & rd1[,5] > rd1[,4]) | (rd1[,9]<0 & rd1[,3] < rd1[,2] & rd1[,4] < rd1[,3] &  rd1[,5] < rd1[,4] )) ,]
dim(rd2)
ns<-sample(nrow(rd2),5000)
read0<-rd2[ns,]
trajM =  traj_Transformation(read0, sname_pos = 1, D0_pos = 2, fitlm_pos = 9)
traj_Plot(trajM, paste("tgf/fitness/",cond, ".lineage_trajectories.pdf", sep=""))
...
perM_cal = function(cond){
  rpm = read.csv(file=paste("tgf/fitness/",cond, ".RPM_fitlm.csv", sep=""), header =T, row.names = 1)
...
perM.1 = perM_cal(cond = "tgf.1")
perM.2 = perM_cal(cond = "tgf.2")
...
write.csv(perM, "tgf/fitness/tgf_relative_RPM_percentage_SKO_DKO_CTRL.csv")
...
totalcond = c("tgf.1", "tgf.2")
for (cond in totalcond) {
  read0 = read.csv(paste("tgf/fitness/",cond, ".RPM_fitlm.csv", sep=""), header=T, row.names=1)
  ...
  write.csv(fitM, file=paste("tgf/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""))
}

#Construct fitness matrix for bootstrapping or permutation
#---------------------------------------------------------------------------------------------------------------------------------
cond = "tgf.1"
fitM0.1 = read.csv(file=paste("tgf/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""), row.names = 1, header = T)
colSums(!is.na(fitM0.1))
fitM1.1 = as.matrix(fitM0.1[,1:49])
dim(fitM1.1)
ctrl.1 = t(fitM0.1[which(rownames(fitM0.1) == "CTRL-CTRL"), 1:11]) #Discard NAs
CTRL.1 = apply(ctrl.1, 1, function(x){return (rep(x, nrow(fitM0.1)))})
dim(CTRL.1)

cond = "tgf.2"
fitM0.2 = read.csv(file=paste("tgf/fitness/", cond, ".fitlm.Matrix.1379_gene_pairs.csv", sep=""), row.names = 1, header = T)
colSums(!is.na(fitM0.2))
fitM1.2 = as.matrix(fitM0.2[,1:49])
ctrl.2 = t(fitM0.2[which(rownames(fitM0.2) == "CTRL-CTRL"),1:10]) #Discard NAs
CTRL.2 = apply(ctrl.2, 1, function(x){return (rep(x, nrow(fitM0.2)))})

fitM = cbind(fitM1.1, fitM1.2[rownames(fitM1.1),], CTRL.1, CTRL.2)
dim(fitM)
write.csv(fitM, file="tgf/fitness/tgf.combined.fitMatrix_1379_gene_pairs.csv")

#---------------------------------------------------------------------------------------------------------------------------------
nBoot = 1000
cond = "tgf.1"
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for tgf.1
Boot1 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for tgf.1
idxF1 = 1:49
...
write.csv(RES1, file=paste("tgf/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))

#---------------------------------------------------------------------------------------------------------------------------------
cond = "tgf.2"
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for tgf.2
Boot2 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for tgf.2
idxF2 = 50:98
...
write.csv(RES2, file=paste("tgf/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))

#---------------------------------------------------------------------------------------------------------------------------------
cond = "tgf.combined"

#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with 0) for tgf.1 and tgf.2
Boot3 = NULL
for(i in 1:nBoot){
...
#Bootstrapping to calculate the 95% CI, and significance of mean fitness (compared with fitness of CTRL) for tgf.1 and tgf.2
idxF3 = 1:98
idxR3 = 99:ncol(fitM)
...
write.csv(RES3, file=paste("tgf/fitness/", cond, ".fitlm_se_CI_diff_p_1379_gene_pairs.csv", sep=""))
...
for (cond in totalcond) {
  ...
  write.csv(wholeM, file=paste("tgf/fitness/", cond, "_fitMatrix_SKO_sgRNAs.csv", sep=""))
}
...
cond = 'tgf.2'
read0 = read.csv(file = paste0("tgf/fitness/",cond,".RPM_fitlm.csv"), header=T, row.names=1)
wholeM = as.matrix(read.csv(file = paste0("tgf/fitness/",cond,"_fitMatrix_SKO_sgRNAs.csv"), header=T, row.names=1))
...
write.csv(GGIvalues, file=paste("tgf/GI/", cond, "_GI_sgRNA_level.csv",sep=""))
...
write.csv(total, file=paste("tgf/GI/", cond, "_GI_gene_level.csv",sep=""))
...
Fit = Order_rname(file_name = "tgf/fitness/tgf.combined.fitlm_se_CI_diff_p_1379_gene_pairs.csv")
dim(Fit)                                   
write.csv(Fit, "tgf/fitness/final_tgf_combined_fitlm_se_CI_diff_p_1379_gene_pairs_ordered.csv")

#GI score file: Change rownames by alphabetic order
GI_tgf.1 = Order_rname(file_name = "tgf/GI/tgf.1_GI_gene_level.csv")
colnm = colnames(GI_tgf.1)
colnames(GI_tgf.1) = paste0("Rep1_", colnm)
dim(GI_tgf.1)
GI_tgf.1 = GI_tgf.1[order(rownames(GI_tgf.1)),]
write.csv(GI_tgf.1, "tgf/GI/final_tgf.1_GI_gene_level_ordered.csv")

GI_tgf.2 = Order_rname(file_name = "tgf/GI/tgf.2_GI_gene_level.csv")
colnm = colnames(GI_tgf.2)
colnames(GI_tgf.2) = paste0("Rep2_", colnm)
dim(GI_tgf.2)
GI_tgf.2 = GI_tgf.2[order(rownames(GI_tgf.2)),]
write.csv(GI_tgf.2, "tgf/GI/final_tgf.2_GI.gene_level_ordered.csv")

#Combine fitness and GI score
all(rownames(GI_tgf.1) == rownames(GI_tgf.2))
Res = cbind.data.frame(Fit[rownames(GI_tgf.1),],GI_tgf.1, GI_tgf.2)
...
write.csv(Res, "tgf/Final_tgf_f.avg_GI.csv")
...
fit = read.csv("tgf/fitness/final_tgf_combined_fitlm_se_CI_diff_p_1379_gene_pairs_ordered.csv", row.names = 1, header = T)
dat = data.frame(fit[grepl("CTRL", rownames(fit)),])
dat$sname = unlist(lapply(rownames(dat), function(x) strsplit(x, split = "-")[[1]][1]))
write.csv(dat, "tgf/fitness/tgf_SKO_fitness_gene_level.csv")
...
p <- ggplot(dat,aes(x=sname,y=f.avg))+
  ...
  coord_flip()+
  labs(title=paste0("TGF", "\u03B2" , "-1"),x ="SKOs", y = "Fitness")
p
ggsave("tgf/fitness/tgf_SKO_fitness_53_genes.pdf", width = 3.6, height= 9, device=cairo_pdf)

#----------------------------------II. Correlation of gene-level fitness of two replicates--------------------------------
f1 = read.csv("tgf/fitness/tgf.1.fitlm_se_CI_diff_p_1379_gene_pairs.csv", row.names = 1, header = T)
f2 = read.csv("tgf/fitness/tgf.2.fitlm_se_CI_diff_p_1379_gene_pairs.csv", row.names = 1, header = T)
...
ggscatter(df, x = "rep1" , y = "rep2", size = 3, alpha = 0.5,
          ...
          cor.coeff.args = list(method = "pearson", label.x = -0.27,label.y = 0.25, color = "blue",label.sep = "\n", size = 7))+
  theme(#panel.border = element_blank(), 
    panel.grid = element_blank(),
    panel.background = element_blank(),
    #panel.grid= element_line(linetype = "dashed"),
  ...
  scale_x_continuous(limits = c(-0.3,0.3), breaks = round(seq(-0.3,0.3,0.2),2))+
  scale_y_continuous(limits = c(-0.3,0.3), breaks = round(seq(-0.3,0.3,0.1),2))+
  geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
  geom_vline(xintercept= 0, linetype="dashed", color = "grey")+
  labs(title=paste0("TGF", "\u03B2" , "-1"),x ="Fitness, Rep 1", y = "Fitness, Rep 2")

ggsave("tgf/fitness/Reproducibility_gene_level_fitnesss_tgf.pdf", width =4.4, height = 4, device=cairo_pdf)

#---------------------------------- III. Plotting oncogenic GIs--------------------------------
res=  data.frame(read.csv("tgf/Final_tgf_f.avg_GI.csv", header=T, row.names=1))
...
write.csv(PPI_net, "tgf/GI/Oncogenic_GIs_cytoscape_tgf.csv", quote = FALSE)
...
p <- ggplot(res, aes(x = gi.Weighted.Avg.Max, y = gi.Weighted.Avg.Min)) +
  ...
  labs(title=paste0("Growth-promoting interactions","\n",(paste0("+ TGF ","\u03B2", "1"))),x ="Mean: f(DKO) - f(SKO1)", y = "Mean: f(DKO) - f(SKO2)")+
  scale_x_continuous(limits =c(-0.25,0.12), breaks = round(seq(-0.3, 0.1, 0.1),2)) +
  scale_y_continuous(limits= c(-0.1,0.4), breaks = round(seq(-0.1, 0.4, 0.1),2))+
  geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
  geom_vline(xintercept= 0, linetype="dashed", color = "grey")
p
require("ggrepel")
...
ggsave("tgf/GI/GI_scatter_plot_tgf.pdf", width = 6.5, height = 7)

#---------------------------------- IV. Correlation of gene-level weighted GI scores of two replicates--------------------------------
res=  data.frame(read.csv("tgf/Final_tgf_f.avg_GI.csv", header=T, row.names=1))
ggscatter(res, x = "Rep1_Max.mean" , y = "Rep2_Max.mean", size = 3, alpha =0.5,
          add = "reg.line", conf.int = FALSE, 
          add.params = list(color = "#007AFF", fill = "lightgray"), # Customize reg. line
          cor.coef = TRUE,rug = TRUE,
          cor.coeff.args = list(method = "pearson", label.x = -0.29,label.y = 0.06, color = "blue",label.sep = "\n", size = 7))+
  theme(#panel.border = element_blank(), 
    panel.grid = element_blank(),
    panel.background = element_blank(),
    #panel.grid= element_line(linetype = "dashed"),
  ...
  scale_x_continuous(limits = c(-0.3,0.1), breaks = round(seq(-0.3,0.1,0.2),2))+
  scale_y_continuous(limits = c(-0.3,0.1), breaks = round(seq(-0.3,0.1,0.1),2))+
  geom_hline(yintercept= 0, linetype="dashed", color = "grey")+
  geom_vline(xintercept= 0, linetype="dashed", color = "grey")+
  labs(title=paste0("TGF", "\u03B2" , "-1"),x ="GI score, Rep 1", y = "GI score, Rep 2")

ggsave("tgf/GI/Reproducibility_gene_level_GI_score_tgf.pdf", width = 4.4, height = 4, device=cairo_pdf)

#---------------------------------- V. GI profiles similarity heatmap--------------------------------
#Color brewer
numericToColor = function(x, mycol = c("purple", "grey80", "green"), mybreaks = seq(-0.1, 0.1, 0.001), bkground = "grey80") {
...
##Input matrix
input_s = read.csv("tgf/Final_tgf_f.avg_GI.csv", row.names=1)
...
for (svalue in totalvalues) {
  ...
  write.csv(GImatrix, file=paste("tgf/GI/", svalue, ".GIMatrix.csv",sep=""))
  write.csv(corrMatrix, file=paste("tgf/GI/", svalue, ".correlation.Based.On.GI.csv",sep=""))
  
  ##Determine color of sinlge-gene fitness
  f0 = data.frame(read.csv("tgf/fitness/tgf_SKO_fitness_gene_level.csv", row.names = 1, header = T))
  ...
  #GI profiling 
  pdf(file = paste("tgf/GI/", svalue, ".correlation.Based.On.GI.pdf",sep=""),width=15, height=15)
...
ggplot(df, aes(X1, X2, fill = value)) + 
  geom_raster()+ 
  scale_fill_gradientn(name = "GI correlation (r)",
                       colours=my.colors,na.value = "transparent",
                       breaks=seq(-0.5,1,0.5),labels=seq(-0.5,1,0.5),
                       limits=c(-0.5,1))+
  guides(size = 3, fill = guide_colourbar(ticks = TRUE))+
  theme(legend.text = element_text(size=14),
        legend.title = element_text(size = 14))
ggsave("tgf/GI/tgf_scale_color_GI_correlation_r.pdf", width = 3, height =3)
...
ggsave("tgf/GI/tgf_scale_color_single_gene_fitness.pdf", width = 3, height =3)
